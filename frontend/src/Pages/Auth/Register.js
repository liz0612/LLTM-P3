...
const Register = () => {
    ...
    const handleSubmit = async (e) => {
      ...
      const { username, password } = values; // Ensure `username` and `password` are correctly captured
    
      setLoading(true);
    
      try {
        const response = await axios.post("http://localhost:3000/signup", {
          username, // Make sure `username` is populated
          password, // Make sure `password` is populated
        });
